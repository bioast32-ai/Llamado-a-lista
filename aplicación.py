...
def xml_report_to_attendance(file_bytes: bytes):
    ...
    # Buscar periodo, por ejemplo 2026.09.01~2026.09.30
    ...
    while i < len(rows):
        row = rows[i]
        # Estructura: ID: | 00001 | Nombre: | ... | Sector: | ...
        if at(row, 0) == "ID:" and at(row, 2) == "Nombre:":
            ...
            for day_offset in range(days_in_period):
                ...
                # Extraer HH:MM, quitando duplicados exactos y manteniendo orden.
...
def generic_excel_to_attendance(file_bytes: bytes, filename: str):
    """Fallback para archivos Excel tabulares comunes."""
...
# Fecha por defecto: la más reciente con al menos una llegada; si no, inicio del periodo.
...
# Garantizar que aparezca toda la lista aunque el formato de entrada fuese tabular e incompleto.
...
m1.metric("Personas", len(view))
m2.metric("Llegaron", arrived)
m3.metric("Sin marcación", pending)
m4.metric("Asistencia", f"{rate:.1f}%")

if arrived:
    st.caption(f"Llegadas tardías según {expected_time.strftime('%H:%M')} + {int(tolerance)} min de tolerancia: {late}")

# =========================
# GRÁFICAS DE LLEGADAS
# =========================
st.subheader("📊 Gráficas de llegadas")

chart_data = attendance.copy()
chart_data["Fecha"] = pd.to_datetime(chart_data["Fecha"], errors="coerce")
chart_data["Llegada_dt"] = pd.to_datetime(
    chart_data["Llegada"].replace("", pd.NA),
    format="%H:%M",
    errors="coerce"
)
chart_data["Hora"] = chart_data["Llegada_dt"].dt.hour

g1, g2 = st.columns(2)

with g1:
    st.markdown("**Llegadas por día**")
    daily_arrivals = (
        chart_data.assign(
            Llegó=chart_data["Llegada"].astype(str).str.strip().ne("")
        )
        .dropna(subset=["Fecha"])
        .groupby("Fecha")["Llegó"]
        .sum()
    ...
    daily_arrivals = daily_arrivals.reindex(fechas, fill_value=0)
    daily_arrivals.index.name = "Fecha"

    st.bar_chart(daily_arrivals, use_container_width=True)

with g2:
    st.markdown("**Distribución de llegadas por hora**")
    hourly_arrivals = (
        chart_data.dropna(subset=["Hora"])
        .groupby("Hora")
        .size()
        .rename("Llegadas")
    ...
    hourly_arrivals.index.name = "Hora"

    st.bar_chart(hourly_arrivals, use_container_width=True)

st.markdown("**Resumen de la fecha seleccionada**")
status_chart = pd.DataFrame(
    {
        "Cantidad": [arrived, pending, late]
    },
    index=["Llegaron", "Sin marcación", "Tardanzas"]
)
...
with tab1:
    ...
    st.dataframe(arrived_df, use_container_width=True, hide_index=True)
with tab2:
...
st.download_button(
    ...
    file_name=f"asistencia_{selected_date.isoformat()}.xlsx",
