...
# =========================================================
# DISEÑO Y ESTILOS PERSONALIZADOS - AZUL OSCURO (DARK MODE)
# =========================================================
st.markdown("""
    <style>
    /* Fondo principal de la aplicación en Azul Oscuro Profundo */
    .stApp {
    ...
    /* Botón principal para descarga de Excel */
...
def get_avatar_url(person_id):
    # Genera una foto tipo avatar generado por IA (DiceBear Avataaars) basado en el ID único
...
def to_excel_bytes(df):
    output = io.BytesIO()
    # Para el Excel descargable removemos la columna Foto para no corromper la descarga
    excel_df = df.copy()
    if "Foto" in excel_df.columns:
        excel_df = excel_df.drop(columns=["Foto"])
    try:
        ...
        excel_df.to_csv(output, index=False, encoding="utf-8-sig")
...
# Agregar la columna 'Foto' con avatares de IA al final
view["Foto"] = view["ID"].apply(get_avatar_url)
...
st.write("---")
st.subheader(f"📊 Gráficas de Asistencia del día ({selected_date.strftime('%d/%m/%Y')})")

g1, g2 = st.columns(2)

with g1:
    st.markdown("**Estado general (Quiénes llegaron vs No llegaron)**")
    summary_df = pd.DataFrame(
        {
            "Cantidad": [on_time, late, pending]
        },
        index=["Llegó A Tiempo", "Llegó Tarde", "Sin marcación"]
    )
    st.bar_chart(summary_df)

with g2:
    st.markdown("**Asistencia por Sector / Curso**")
    if "Sector" in view.columns and not view["Sector"].dropna().empty:
        ...
        st.info("No hay información de sectores/grupos en la lista.")
...
export_cols = ["Fecha", "ID", "Nombre", "Sector", "Llegada", "Salida", "Puntualidad", "Estado", "Marcaciones", "Foto"]
st.dataframe(
    filtered[export_cols],
    column_config={
        "Foto": st.column_config.ImageColumn("Foto / Avatar (IA)", help="Avatar automático de IA"),
    },
    hide_index=True,
)
...
with tab1:
    arrived_df = view[view["Estado"] == "Llegó"][["ID", "Nombre", "Sector", "Llegada", "Puntualidad", "Foto"]]
    st.dataframe(arrived_df, column_config={"Foto": st.column_config.ImageColumn("Foto")}, hide_index=True)
with tab2:
    pending_df = view[view["Estado"] == "Sin marcación"][["ID", "Nombre", "Sector", "Puntualidad", "Foto"]]
    st.dataframe(pending_df, column_config={"Foto": st.column_config.ImageColumn("Foto")}, hide_index=True)
with tab3:
    late_df = view[view["Puntualidad"] == "Tarde"][["ID", "Nombre", "Sector", "Llegada", "Foto"]]
